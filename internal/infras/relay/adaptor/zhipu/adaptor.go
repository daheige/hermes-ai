...
func (a *Adaptor) ConvertRequest(c *gin.Context, relayMode int, request *model2.GeneralOpenAIRequest) (any, error) {
	...
	default:
		// TopP [0.0, 1.0]
		request.TopP = utils.Float64PtrMax(request.TopP, 1)
		request.TopP = utils.Float64PtrMin(request.TopP, 0)

		// Temperature [0.0, 1.0]
		request.Temperature = utils.Float64PtrMax(request.Temperature, 1)
		request.Temperature = utils.Float64PtrMin(request.Temperature, 0)
...
func (a *Adaptor) DoResponseV4(c *gin.Context, resp *http.Response, meta *meta.Meta) (usage *model2.Usage, err *model2.ErrorWithStatusCode) {
	if meta.IsStream {
...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, meta *meta.Meta) (usage *model2.Usage, err *model2.ErrorWithStatusCode) {
	...
	case relaymode.Embeddings:
		...
		return
	case relaymode.ImagesGenerations:
		err, usage = openai2.ImageHandler(c, resp)
		return
	}
	if a.APIVersion == "v4" {
...
func ConvertEmbeddingRequest(request model2.GeneralOpenAIRequest) (*EmbeddingRequest, error) {
