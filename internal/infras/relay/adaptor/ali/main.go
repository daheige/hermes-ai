...
func ConvertRequest(request model2.GeneralOpenAIRequest) *ChatRequest {
	...
	request.TopP = utils.Float64PtrMax(request.TopP, 0.9999)
	return &ChatRequest{
		Model: aliModel,
		Input: Input{
			Messages: messages,
...
func ConvertImageRequest(request model2.ImageRequest) *ImageRequest {
...
func embeddingResponseAli2OpenAI(response *EmbeddingResponse) *openai2.EmbeddingResponse {
	openAIEmbeddingResponse := openai2.EmbeddingResponse{
		Object: "list",
		Data:   make([]openai2.EmbeddingResponseItem, 0, len(response.Output.Embeddings)),
		Model:  "text-embedding-v1",
		Usage:  model2.Usage{TotalTokens: response.Usage.TotalTokens},
	}
...
func responseAli2OpenAI(response *ChatResponse) *openai2.TextResponse {
...
func streamResponseAli2OpenAI(aliResponse *ChatResponse) *openai2.ChatCompletionsStreamResponse {
...
func Handler(c *gin.Context, resp *http.Response) (*model2.ErrorWithStatusCode, *model2.Usage) {
	...
	slog.With("request_id", logger.GetRequestID(ctx)).
		Debug(fmt.Sprintf("response body: %s\n", responseBody))
