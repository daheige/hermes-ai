...
func requestOpenAI2Xunfei(request model2.GeneralOpenAIRequest, xunfeiAppId string, domain string) *ChatRequest {
	...
	if strings.HasPrefix(domain, "generalv3") || domain == "4.0Ultra" {
		functions := make([]model2.Function, len(request.Tools))
...
func getToolCalls(response *ChatResponse) []model2.Tool {
...
func responseXunfei2OpenAI(response *ChatResponse) *openai2.TextResponse {
	if len(response.Payload.Choices.Text) == 0 {
...
func streamResponseXunfei2OpenAI(xunfeiResponse *ChatResponse) *openai2.ChatCompletionsStreamResponse {
...
func buildXunfeiAuthUrl(hostUrl string, apiKey, apiSecret string) string {
	...
	if err != nil {
		log.Println("failed to parse host url,err:", err)
	}
...
func StreamHandler(c *gin.Context, meta *meta.Meta, textRequest model2.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*model2.ErrorWithStatusCode, *model2.Usage) {
	domain, authUrl := getXunfeiAuthUrl(meta.Config.APIVersion, apiKey, apiSecret)
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId)
	if err != nil {
		return openai2.ErrorWrapper(err, "xunfei_request_failed", http.StatusInternalServerError), nil
	}
	...
	c.Stream(func(w io.Writer) bool {
		...
		case xunfeiResponse := <-dataChan:
			...
			c.Render(-1, customevent.CustomEvent{Data: "data: " + string(jsonResponse)})
			return true
		case <-stopChan:
...
func Handler(c *gin.Context, meta *meta.Meta, textRequest model2.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*model2.ErrorWithStatusCode, *model2.Usage) {
	domain, authUrl := getXunfeiAuthUrl(meta.Config.APIVersion, apiKey, apiSecret)
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId)
	if err != nil {
		return openai2.ErrorWrapper(err, "xunfei_request_failed", http.StatusInternalServerError), nil
	...
	if len(xunfeiResponse.Payload.Choices.Text) == 0 {
		return openai2.ErrorWrapper(errors.New("xunfei empty response detected"), "xunfei_empty_response_detected", http.StatusInternalServerError), nil
	}
...
func xunfeiMakeRequest(textRequest model2.GeneralOpenAIRequest, domain, authUrl, appId string) (chan ChatResponse, chan bool, error) {
	d := websocket.Dialer{
	...
	go func() {
		for {
			...
			if response.Payload.Choices.Status == 2 {
				err := conn.Close()
				if err != nil {
					slog.Error("error closing websocket connection: " + err.Error())
				}
				break
			}
		}
