...
func ConvertRequest(textRequest model2.GeneralOpenAIRequest) *Request {
	...
	for _, message := range textRequest.Messages {
		if message.Role == "system" && claudeRequest.System.IsEmpty() {
			systemBytes, _ := json.Marshal(message.StringContent())
			claudeRequest.System = SystemContent(systemBytes)
			continue
		}
		...
		if message.IsStringContent() {
			...
			claudeMessage.Content = contentBytes
...
func StreamResponseClaude2OpenAI(claudeResponse *StreamResponse) (*openai2.ChatCompletionsStreamResponse, *Response) {
	var response *Response
	var responseText string
	var stopReason string
	tools := make([]model2.Tool, 0)
	...
	case "content_block_start":
		if claudeResponse.ContentBlock != nil {
			...
			if claudeResponse.ContentBlock.Type == "tool_use" {
				tools = append(tools, model2.Tool{
...
func ResponseClaude2OpenAI(claudeResponse *Response) *openai2.TextResponse {
	var responseText string
...
func StreamHandler(c *gin.Context, resp *http.Response) (*model2.ErrorWithStatusCode, *model2.Usage) {
	...
	var id string
	var lastToolCallChoice openai2.ChatCompletionsStreamResponseChoice
