...
func ImageHandler(c *gin.Context, resp *http.Response) (*model.ErrorWithStatusCode, *model.Usage) {
	...
	for {
		err = func() error {
			...
			respBody := &openai2.ImageResponse{
				Created: taskData.CompletedAt.Unix(),
				Data:    []openai2.ImageData{},
			}
			...
			for _, imgOut := range output {
				...
				pool.Go(func() error {
					...
					mu.Lock()
					respBody.Data = append(respBody.Data, openai2.ImageData{
			...
			if err := pool.Wait(); err != nil {
				...
				slog.With("request_id", logger.GetRequestID(c.Request.Context())).
					Error(fmt.Sprintf("some images failed to download: %+v", err))
			}
		...
		if err != nil {
			...
			return openai2.ErrorWrapper(err, "image_task_failed", http.StatusInternalServerError), nil
		}
